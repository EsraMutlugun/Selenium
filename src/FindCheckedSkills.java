...
public class FindCheckedSkills {
	...
	@Test
	// listedeki checked edilmis kutucuklari  bulmak icin 1. Yol 
	public void testCheckedSkills() {
		driver.get("http://localhost:8080/workspace/xpath.html");
		// (>)  isareti CSS syntaxten geliyor ve ul altindaki listeyi(first child) aliyor eger > isareti olmasaydi alt listeteki child listleride getirirdi.
		...
		for (WebElement skill: skills){
			// Is there any a checked check box fakat eger checked box isaretli degilse ne olacak? Ozaman try kismini kullanmak lazim
			try {
	...
	//checked box kisimlari farkli sekilde yazilmis olabilir. checked='checked' ya da checked='true' ya da checked=true ya da sadece
	...
	public void testCheckedSkills2() {
		...
		for (WebElement skill: skills){
			...
			// checkbox larin isaretli olup olmadigini tek tek kontrol edecek. null yazarsa isaretli degil, true yazarsa isaretli
	...
	public void testCheckedSkillsByXPath() {
		...
		String[] expected = {"Java", "C#"};
		List<String> actual = new ArrayList();
		for (WebElement li: checkedLis) {actual.add(li.getText());
